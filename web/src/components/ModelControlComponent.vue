...
export default {
  ...
  methods: {
    async update_definition() {
      const url = `${this.uiConfig.settings.apiUrl}/api/definitions/update_definition?token=${this.user.token}`;
    ...
    async update_engine() {
      const url = `${this.uiConfig.settings.apiUrl}/api/engines/update_engine?token=${this.user.token}`;
      let response = await fetch(url, {
        method: "POST",
        headers: {
          Accept: "application/json, text/plain, */*",
