...
import explain from "./gas_container.png";
...
class DiagramGasCompartment {
  ...
  draw(stage, rtData) {
    ...
    this.sprite.tint = this.CalculateColor(
      this.sprite.co2 / this.sprite.modelComponents.length
    );
    this.sprite.scale.set(
      this.sprite.volume * this.sprite.scalingFactorX * this.sprite.globalScale,
      this.sprite.volume * this.sprite.scalingFactorY * this.sprite.globalScale
    );
  }
  ...
  onDragEnd(e) {
    ...
    const closestX = this.x_grid.reduce((a, b) => {
      return Math.abs(b - this.interactionData.global.x) <
        Math.abs(a - this.interactionData.global.x)
        ? b
        : a;
    });
    const closestY = this.y_grid.reduce((a, b) => {
      return Math.abs(b - this.interactionData.global.y) <
        Math.abs(a - this.interactionData.global.y)
        ? b
        : a;
    });
    switch (this.editMode.mode) {
    ...
    switch (this.editMode.mode) {
      ...
      case 3: // morphing
        if (this.morphing) {
          ...
          this.scale.set(
            this.volume * this.scalingFactorX * this.globalScale,
            this.volume * this.scalingFactorY * this.globalScale
          );
          this.prevX = this.interactionData.global.x;
        }
        break;
